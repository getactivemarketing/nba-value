...
class MLBScorer:
    ...
    def __init__(
        ...
        run_diff_path = Path(
            run_diff_model_path
            or settings.mlb_run_diff_model_path
            or self.DEFAULT_RUN_DIFF_MODEL
        )
    ...
    async def _calculate_market_values(
        ...
        prediction.best_bet = MLBValueCalculator.find_best_bet(
            all_values,
            include_totals=settings.totals_in_best_bet,
            include_runline=settings.runline_in_best_bet,
        )
